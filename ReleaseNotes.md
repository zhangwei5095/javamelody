### Next release (master) ###
  ...
  * fix: check if async before flushing the response, ([2bb18a6](../../commit/2bb18a6f2d7aefdbf8eda489fd68f57487e67570) thanks to _Mark Thomas_)
  * fix [issue 338](../../issues/338): JavaMelody swallows response when some Spring WebAsyncTask/Callable is used (thanks to _Violeta Georgieva_). The [web.xml example](../wiki/UserGuide.md#2-webxml-file) in the User's guide is also changed for Async requests with Servlet API 3.0.
  * fix: in latest JIRA versions, monitoring report outputs garbage (gzip compression is now enabled by default in JIRA)

### 1.56.0 ###

  * fix [issue 476](../../issues/476): ClassLoader leak when using the reports and then hot-redeploy with Tomcat 7.0.42 or later
  * fix [issue 475](../../issues/475): IllegalAccessException when creating Jdbc Proxy (Java 8)
  * fix [issue 473](../../issues/473): Basic Authentication and Sessions
  * fix [issue 460](../../issues/460): In WildFly 8.2, "ClassNotFoundException: Can't find a delegate" when monitoring JPA
  * fix [issue 477](../../issues/477): In the processes list, CPU and Command are sometimes in the wrong column
  * added: MULTILINE & DOTALL flags for transform-patterns regexps ([issue 474](../../issues/474), thanks to _Michal Bergmann_).
  ...
  * doc updated: Examples of web.xml and of context-param updated in the [user's guide](UserGuide#2._web.xml_file.md) to be sure that parameters are not ignored in some case of webapp v3.1 with JDK8 ([issue 459](../../issues/459)).
...
### 1.55.0 ###
  ...
  * fix ~~[issue 104](../../issues/104#c14)~~ again, for recent TomEE versions (thanks to _Frederic Cornuau_)
  * fix [issue 436](../../issues/436): implement Servlet 3.1 new methods (JavaEE 7), in FilterServletOutputStream and others
  * fix [issue 453](../../issues/453): Chinese translation for heap dump (thanks to _chuxuebao_)
  * fix [issue 455](../../issues/455): HTTP-401 / WWW-Authenticate wrongly reported as HTTP error

### 1.54.0 ###

  * fix [issue 440](../../issues/440): Not able to start Desktop version.
  ...
  * improved: In the optional collector server, when the monitoring-path parameter such as /admin/monitoring is used in the monitored webapp(s), support the same parameter for calling the webapp(s) from the collector server, for example -Djavamelody.monitoring-path=/admin/monitoring ([issue 442](../../issues/442), thanks to _Dimitris Mouchritsas & Manolis Pitarokilis_)
...
### 1.53.0 ###
  ...
  * fix: in v1.52.0 with Tomcat, graphs of bytes received/sent and of Tomcat active threads were not displayed anymore in "Other charts" ([revision 3908](https://code.google.com/p/javamelody/source/detail?r=3908)). This was unfortunately caused by the fix of [issue 406](../../issues/406).
  * fix [issue 342](../../issues/342): In JIRA logs since JIRA v6.0.8, "RuntimeException converting item 'net.bull.javamelody:net.bull.javamelody.jira-web-item' to Simple link"
  * fix [issue 439](../../issues/439): Display Linux version in "System informations", and not "Linux unknown"
  ...
  * added: new parameter "sampling-included-packages" for a white list in [cpu hotspots](UserGuideAdvanced#Enable_Hotspots_detection.md), instead of using the "sampling-excluded-packages" parameter ([issue 424](../../issues/424), thanks to _alf.hogemark_)
...
### 1.52.0 ###
  ...
  * fix [issue 406](../../issues/406): Slow GUI with JBoss EAP 6.2 (only since 6.2)
  * fix [issue 415](../../issues/415): Periodic stack trace in Tomcat 8 logs 'java.lang.IllegalStateException: The resources may not be accessed if they are not currently started'
  * fix [issue 423](../../issues/423): Link to DataSource Reference Returns HTTP 404
  ...
  * fix [issue 377](../../issues/377): in JIRA, Confluence: Incorrect Installation Instructions on Atlassian Marketplace
...
### 1.51.0 ###
  ...
  * fix [issue 396](../../issues/396): Call to TraversableResolver.isReachable() throws exception "Can't find a delegate"
  * fix [issue 399](../../issues/399): Report of large Nb of used jdbc connections with Spring Routing Datasource
  ...
  * improved: abstract very long sql requests ([issue 403](../../issues/403))
  * improved: in a graph detail, a checkbox can now hide maximum values in the graph, so that average values are better displayed when much lower than the maximum ([issue 368](../../issues/368))
...
### 1.50.0 ###
  ...
  * fix [issue 370](../../issues/370): work around ConcurrentModificationException during Tomcat startup (which is a [Tomcat bug](https://issues.apache.org/bugzilla/show_bug.cgi?id=56082))
  * fix [issue 378](../../issues/378): Slf4j system, with log4j-over-slf4j, is not detected with version 1.7.6 of slf4j (thanks to _jgraglia_)
  ...
  * fix [issue 386](../../issues/386): IllegalArgumentException: No enum const class ..., in Turkish
...
### 1.49.0 ###
  ...
  * fix [issue 367](../../issues/367): soap error, in java 1.5
  ...
  * fix [issue 361](../../issues/361): InvocationTargetException when Accessing Monitoring URL on JIRA, with v1.48.0
...
### 1.48.0 ###
  ...
  * fix [issue 359](../../issues/359): Support Confluence 5.3
  * fix [issue 354](../../issues/354): With the java update 1.7\_u45, the **Desktop app** of the monitoring does not start
  * fix [issue 355](../../issues/355): ClassNotFoundException when serializing session breaks the sessions view
  * fix [issue 350](../../issues/350): Hotspots are not being gathered when using centralized monitoring (with help from _kalnas2_)
  ...
  * Optimize jdbc overhead, when there are more than 100000 sql hits per minute ([issue 353](../../issues/353), with help from _kalnas2_)
  * New parameter to optimize opened jdbc connections, when there are more than 100000 sql hits per minute ([issue 352](../../issues/352))
  * display incoming **web-services SOAP** methods in the http statistics ([issue 228](../../issues/228), thanks to _roy.paterson_)

### 1.47.0 ###

  * fix [issue 339](../../issues/339): "One day" memory leak
  * fix [issue 346](../../issues/346): XSS through X-Forwarded-For header spoofing
  * fix [issue 328](../../issues/328): NoSuchFieldError: readOnly when using v1.46 with jrobin 1.5.14 (instead of jrobin 1.5.9 as documented)
  * fix [issue 335](../../issues/335): centralized server not compatible with ehcache 2.7.2
  * fix [issue 232](../../issues/232): Use the path of -XX:HeapDumpPath=/tmp if defined, for the directory of heap dump files (otherwise use the temp directory of the server as before)
  ...
  * added: **detect cpu hotspots** ([issue 149](../../issues/149), with some ideas from _Cédric Lime_).
...
### 1.46.0 ###
  ...
  * fix [issue 321](../../issues/321): javamelody disables Tomcat 7 distributed sessions when using SimpleTcpCluster
  * fix [issue 96](../../issues/96): fix redeployment in tomcat when javamelody and jrobin jar files are in tomcat/lib and fix redeployment in WebLogic Server
  * fix [issue 313](../../issues/313): NoClassDefFoundError when IBM's JVM is used
...
### 1.45.2 (JIRA / Confluence / Bamboo plugin) ###
  ...
  * fix [issue 317](../../issues/317): compatibility of the plugin with JIRA 5.1 and 5.2

### 1.45.1 (JIRA / Confluence / Bamboo plugin) ###

  * fix [issue 314](../../issues/314): compatibility of the plugin with JIRA 6
...
> `grails install-plugin grails-melody`
  ...
  * known incompatibility of the JIRA plugin with JIRA 6.0, [issue 314](../../issues/314)
  ...
  * fix [issue 298](../../issues/298): for the database reports, spclocation was removed in postgresql 9.2 and datconfig was removed in postgresql 9.0
  ...
  * fix [issue 303](../../issues/303): SQL monitoring doesn't work on SpringSource tc Runtime
  ...
  * added: button to kill a thread from the current requests ([issue 302](../../issues/302))
...
### 1.44.0 ###
  ...
  * fix [issue 289](../../issues/289): JBoss AS 7 configuration should not be needed
  * fix [issue 293](../../issues/293): Ehcache 2.7.0 doesn't work with javamelody. Note that because of an [ehcache bug](https://jira.terracotta.org/jira/browse/EHC-1010), the value of the cache efficiency (hits/access) displayed by javamelody is incorrect when using ehcache 2.7.0.
  ...
  * added: display of values in the JNDI tree ([issue 287](../../issues/287), thanks to _derjust_)
...
### 1.43.0 ###
  ...
  * fix [issue 274](../../issues/274): Monitoring of SQL requests after hot redeploy in JBoss, Glassfish or WebLogic (thanks to _lastmike_)
  * fix [issue 281](../../issues/281): Database not recognised in Websphere Application Server (thanks to _lastmike_)
  * fix [issue 269](../../issues/269): Compatibility of the JIRA/Confluence/Bamboo plugin with some other plugins
  * fix [issue 270](../../issues/270): French Canadians can't choose a customized period
  * fix: in the current requests, use the chosen period to display the statistics of mean times, besides the elapsed times
  * optimization of SpringDataSourceBeanPostProcessor ([issue 282](../../issues/282), thanks to _lastmike_)
  * added: PDF report of the current requests
  * added: Support for Sybase in the database reports ([issue 276](../../issues/276), thanks to _lastmike_)
  * added: javamelody parameter "locale" to fix the locale of the reports, whatever the language in the browser ([issue 271](../../issues/271), thanks to _xiukongtiao_)
  * added: possibility to clear a cache individually ([issue 283](../../issues/283), thanks to _idrahou_)

### 1.42.0 ###

  * fix [issue 242](../../issues/242): Just after a restart, the statistics of services (spring, ejb, guice) may be incorrectly summed with past values, if they are initialized AFTER the MonitoringFilter
  * fix [issue 261](../../issues/261): Use the thread context-classloader to load the jdbc driver if used directly
  * fix [issue 262](../../issues/262): NullPointerException in MonitoringFilter.doFilter(), when the webapp is undeployed and after a timeout
  * fix [issue 267](../../issues/267): Suppress "IllegalArgumentException: Database type unknown: jdbc:sqlserver"
  * fix [issue 258](../../issues/258): The IText files causes NullPointerException in FlyingSaucer
...
### 1.41.0 ###
  ...
  * [issue 244](../../issues/244) and [issue 245](../../issues/245) were Resin issues when using javamelody and they are fixed in [Resin v4.0.31](http://caucho.com/resin-4.0/changes/changes.xtp#4031Sep122012)
  * fix [issue 252](../../issues/252): Add XSS protection
  * fix [issue 255](../../issues/255): exception while collecting data java.io.FileNotFoundException: Could not open ....rrd [existent](non.md)
  * fix [issue 248](../../issues/248): JsfActionListener sends two requests when javamelody is disabled
  ...
  * added: allow extend servlet security check ([issue 256](../../issues/256))
...
### 1.40.0 ###
  ...
  * fix [issue 231](../../issues/231): In [JIRA/Confluence/Bamboo plugin](https://plugins.atlassian.com/plugins/net.bull.javamelody), incompatibility with Bamboo remote agents.
  * fix [issue 233](../../issues/233): The monitoring reports should not set autoCommit(false) on JDBC connections obtained from datasources.
  * fix [issue 243](../../issues/243): Spring config files may trigger internet access
...
### 1.39.0 ###
  ...
  * fix [issue 230](../../issues/230): NoClassDefFoundError: javax/servlet/AsyncContext in Tomcat 5.5
...
### 1.38.0 ###
  ...
  * fix major [issue 138](../../issues/138): It is now possible to monitor SQL requests and jdbc connections with a JNDI DataSource in GlassFish v3, (thanks to _Pieter_ for the analysis and to _Bhun Kho_ for the initial workaround). This can work with whatever version of JavaMelody from 1.23 to the latest. Documentation is [here](UserGuideAdvanced#Monitoring_of_sql_requests_and_of_jdbc_connections_in_v3+.md)
  * fix [issue 229](../../issues/229): In Glassfish v2.1.1, Exception in NamingManagerImpl copyMutableObject() for a JNDI DataSource
  * fix [issue 217](../../issues/217): AsyncContext.getResponse().getWriter() throws IllegalStateException "getOutputStream() has already been called for this response" in Tomcat
  * fix [issue 198](../../issues/198): In [JIRA/Confluence/Bamboo plugin](https://plugins.atlassian.com/plugins/net.bull.javamelody), monitoring of http sessions was not working after a few minutes or a few hours with JIRA 4.4 or Confluence 4.1
  * fix minor [issue 210](../../issues/210): In [JIRA/Confluence/Bamboo plugin](https://plugins.atlassian.com/plugins/net.bull.javamelody) with a JIRA server, there was sometimes an error in log about "jira-web-item".
  * In JIRA 5.0, changing the dbconfig.xml with a JdbcDriver does not work anymore to monitor jdbc connections and SQL requests: the [user guide](UserGuide#Atlassian_JIRA,_Confluence_and_Bamboo_Plugin.md) has been updated to use a datasource from jndi instead ([issue 225](../../issues/225)).
  * added: When using the [optional collect server](UserGuideAdvanced#Optional_centralization_server_setup.md), enable use of http Basic authentication to secure access to the monitored webapp, for example when the ip-address of the collect server is not known upfront ([issue 214](../../issues/214), thanks to _purnhar_). Documentation is [here](UserGuideAdvanced#5._Security_with_a_collect_server.md).
...
### 1.37.0 ###
  ...
  * fix [issue 207](../../issues/207): Incompatibility with servlet api 2.4 (Tomcat 5.5) in javamelody 1.36.0
  * fix [issue 205](../../issues/205) and [issue 206](../../issues/206): delegate the JSF ActionListener to the next JSF ActionListener if one was defined in the faces-config.xml file (thanks to _Patrick Dobler_)
  ...
  * fix [issue 187](../../issues/187): can't protect Melody grails plugin with spring security core (thanks to _Burt Beckwith_)
  * fix ~~[GPMELODY-4](http://jira.grails.org/browse/GPMELODY-4)~~: Load the Melody grails plugin after shiro (thanks to _Tomas Lin_).
  * added: Display the properties of the new [Tomcat jdbc](http://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html) datasources in System informations like for Tomcat DBCP, in particular display the max number of active connections next to the current number of active connections ([issue 29](../../issues/29)).
...
> Other notes:
    * fix blocking [issue 194](../../issues/194) and [issue 143](../../issues/143), [issue 199](../../issues/199), [issue 200](../../issues/200): The jrobin v1.5.9 artifacts are now available in the **Maven central** repository as you can see [here](http://search.maven.org/#artifactdetails|org.jrobin|jrobin|1.5.9|jar). The external (and currently unreachable) repository in the pom is now useless and removed.
    * fix minor [issue 192](../../issues/192): In a Bamboo server v3.1+, internal server error when viewed by non-admin user.
    * fix [issue 193](../../issues/193): Make "Deployment on Tomcat without modification of monitored webapps" work like before v1.32
    * fix [issue 189](../../issues/189): Should not set autoCommit to false on datasource connections
    * fix [issue 195](../../issues/195): In the database reports, compatibility of "innodb status" with both Mysql 5.5 and Mysql 4
    * fix [issue 201](../../issues/201): Unable to render MBeans tree when some JMX-retrieval exception occurs
    * fix [issue 204](../../issues/204): There should not be any blocking exception when initializing the JSF action listener
    * added: monitoring-spring-datasource.xml, as an alternative to monitoring-spring.xml, to monitor only the datasources when there is some conflict ([issue 172](../../issues/172)). See [doc](UserGuide#7._JDBC.md) at the end of the JDBC chapter.
    * added: Possibility to customize styles, icons and other resources in the html reports ([issue 196](../../issues/196)). See [doc](UserGuideAdvanced#Customizing_styles,_icons_and_other_resources_in_the_html_report.md) in the advanced user guide. Please submit your best [CSS file](http://code.google.com/p/javamelody/source/browse/trunk/javamelody-core/src/main/resources/net/bull/javamelody/resource/monitoring.css) to the [users' group](http://groups.google.com/group/javamelody).
...
### 1.35.0 ###
  ...
  * fix [issue 185](../../issues/185): In [JIRA/Confluence/Bamboo plugin](https://plugins.atlassian.com/plugins/net.bull.javamelody), fix the compatibility of checking the admin permission with Confluence 4.1.4 and after.
  ...
  * fix [issue 178](../../issues/178): In the [Jenkins/Hudson plugin](https://wiki.jenkins-ci.org/display/JENKINS/Monitoring) and in [JIRA/Confluence/Bamboo plugin](https://plugins.atlassian.com/plugins/net.bull.javamelody), since v1.32, "Nb of http sessions" is 0 and "View http sessions" is always empty.
  ...
  * added: some "database reports" for **DB2**, in the "System information" part: MON\_CURRENT\_SQL, MON\_DB\_SUMMARY, MON\_LOCKWAITS, MON\_SERVICE\_SUBCLASS\_SUMMARY, MON\_CURRENT\_UOW, MON\_WORKLOAD\_SUMMARY, MON\_GET\_CONNECTION and fix of SNAPSHOT\_DYN\_SQL ([issue 179](../../issues/179), thanks to _Peter James and his DBA from humanservices.gov.au_)
  * added: optional parameter (dns-lookups-disabled) to allow disabling of dns lookups with InetAddress.getLocalHost() to prevent long hangs on startup/shutdown in some particular environments ([issue 181](../../issues/181), thanks to _r6squeegee_)
  * added: In the [Grails Melody plugin](http://www.grails.org/plugin/grails-melody), read the configuration parameters from both Config.groovy and GrailsMelodyConfig.groovy ([issue 180](../../issues/180), thanks to _padcom_)
  * added: possibility to customize "name" in the statistics (long standing [issue 31](../../issues/31)). Note that instance creation (such as String concatenation) must be reduced to the minimum, otherwise there will be a performance overhead. It is really not recommended to customize.

### 1.34.0 ###

  * fix [issue 121](../../issues/121) and [issue 175](../../issues/175): Incompatibility with **Quartz 2** (thanks to _rogerc at costumercentrix.com_)
  * fix [issue 164](../../issues/164): **JBoss 7** support for SQL monitoring with JNDI datasource (thanks to _sarnowski at new-thoughts.org_)
  * fix [issue 166](../../issues/166): jboss-as-client and javamelody log4j conflict (java.lang.IncompatibleClassChangeError)
  * fix [issue 170](../../issues/170): Plugin for JIRA / Confluence / Bamboo: Compatibility with JIRA 5.0 RC
...
### 1.33.0 ###
  ...
  * fix [issue 146](../../issues/146) and [issue 153](../../issues/153): Incorrect Struts statistics when two actions have the same name but are in different packages. Note: the consequence is that the names of all the Struts actions are changed in the statistics.
  * fix [issue 152](../../issues/152): Error with execution plan for "ALTER SESSION..."
  * fix [issue 157](../../issues/157): In the Grails plugin, do not add groovy meta programming on services if javamelody is disabled. ([rev 76158](http://svn.codehaus.org/grails-plugins/grails-grails-melody/trunk/GrailsMelodyGrailsPlugin.groovy))
  ...
  * fix [issue 160](../../issues/160): Disabling sql statistics with the "displayed-counters" parameter should not disable the connection monitoring
  * added for [issue 158](../../issues/158): note in the UI and in the online help to say that since ehcache v2.1, the cache statistics need to be enabled in order to display efficiency values
...
### 1.32.1 ###
  * fix [issue 151](../../issues/151): a Java 1.6 dependency was introduced in 1.32.0 (NoSuchFieldError: ROOT)
...
### 1.32.0 ###
  ...
  * fix [issue 137](../../issues/137): MonitoringInterceptor when Stateful EJB, Java EE 6
  * fix [issue 141](../../issues/141): exception while collecting data java.io.IOException: Read failed, file xyz not mapped for I/O (after out of disk space)
  * fix [issue 147](../../issues/147): When the monitoring filter is incorrectly defined in web.xml for recent appserver, parameters inside filter are sometimes ignored
  ...
  * added: Chinese translation, and fix display of Chinese characters in the JRobin graphics and in the PDF reports ([issue 150](../../issues/150))
  ...
  * added: Provide an url to test sending a pdf report by mail ([issue 145](../../issues/145))

### 1.31.0 ###
  * fix [issue 128](../../issues/128): Fix the shutdown process in JIRA 4.3.4 and clean it for Confluence/Bamboo/Jenkins/Hudson.
  * fix [issue 129](../../issues/129): ArrayIndexOutOfBoundsException in GWTRequestWrapper in a particular case.
  * fix [issue 134](../../issues/134): In JBoss, sending mails with weekly report should not need jboss-web.xml
  * [issue 133](../../issues/133): [Documentation](UserGuideAdvanced#Usage_of_in_JBoss_AS_7_%28which_uses_OSGI%29.md) added on OSGI configuration when using JBoss AS 7
...
### 1.30.0 ###
  ...
  * fix [issue 116](../../issues/116): MBeans overview does not work with java 1.5
  * fix [issue 117](../../issues/117): Sorting of numbers is based on String comparison in German
  * fix [issue 122](../../issues/122): Average number of requests per minutes seems to be wrong, with a single day in a custom period
  * fix [issue 124](../../issues/124): Start date removed from the text at the top of the monitoring page, except if the "All" period is selected.

### 1.29.0 ###

  * fix [issue 106](../../issues/106): a few HTTP hits are lost in the statistics for the first hit(s) on new requests (no impact on statistics once requests are known)
  * fix [issue 104](../../issues/104): support SQL monitoring in Apache OpenEJB bundled as TomEE
  * fix [issue 107](../../issues/107): Hibernate factory doesn't work with HSQL with batching turned off
  * fix [issue 108](../../issues/108): `async-supported` added in the web-fragment.xml file to support asynchronous requests in Tomcat 7.0.
  * added: PDF report for the "Summary by class"

### 1.28.0 ###

  * fix [issue 99](../../issues/99): NullPointerException when displaying the list of process on AIX
  * fix [issue 101](../../issues/101): In the collector server, can't add an application when the applications.properties file does not exist
  * fix [issue 102](../../issues/102): In the collector server, NoClassDefFoundError when IBM's JVM is used
  * fix [issue 105](../../issues/105): Monitoring JNDI DataSource in Tomcat fails under host name other than "localhost"
...
### 1.27.0 ###
  ...
  * fix [issue 97](../../issues/97): StackOverflowError in JBoss 4 when using JNDI dataSources with several monitored webapps (fix verified by _Saurabh Arora_).
...
### 1.26.0 ###
  ...
  * fix [issue 85](../../issues/85): web-fragment schema location in **JBoss 6.0.0 CE**, patch contributed by _dhartford_
  * fix JNDI report in Glassfish, WebLogic, Jonas and Websphere
  * fix [issue 89](../../issues/89): Listing of MBeans fails in Jetty
...
### 1.25.0 ###
  ...
  * fix [issue 36](../../issues/36): Grails Melody plugin blocks closure (fixed by _Jean Barmash_ and _Liu Chao_)
  * fix [issue 79](../../issues/79): Spring security core integration for the Grails Melody plugin (fixed by _Liu Chao_)
  * fix [issue 80](../../issues/80): Memory histogram should be supported on Mac OS X
  * fix [issue 83](../../issues/83): StringIndexOutOfBoundsException for caches with # in the name of a cache
...
### 1.24.0 ###
  ...
  * fix [issue 67](../../issues/67): EhCache 1.4.x detection is not robust enough (NoClassDefFoundError)
  * fix [issue 68](../../issues/68): support more prefixes for JNDI names of datasources
  * fix [issue 69](../../issues/69): NullPointerException when getting resources if mime-type is null
  * fix [issue 71](../../issues/71): StringIndexOutOfBoundsException with # in JVM parameters
  * fix [issue 73](../../issues/73): / by zero when max\_elements\_in\_memory = 0 in ehcache
  * fix [issue 74](../../issues/74): "View OS Processes" does not work on MAC OS X Server
  * fix [issue 75](../../issues/75): StringIndexOutOfBoundsException for http session with # in attributes
  * added: in the optional collect server, "add and remove applications" can be restricted ([issue 61](../../issues/61))
  * added an enhancement on jdbc proxy: Improve JDBC connection wrapper equals invocation when `args[0]` is a javamelody proxy ([issue 78](../../issues/78))
...
### 1.22.0 ###
  ...
  * fix [issue 54](../../issues/54): v1.21.0 requires jdk 1.6, when "displayed-counters" is used
  * fix [issue 58](../../issues/58): Quartz in Spring and server shutdown. (Added parameter "quartz-default-listener-disabled" to be used for Quartz jobs only when used in Spring. See [doc](UserGuide#12._Batch_jobs_(if_Quartz).md).)
  * fix [issue 59](../../issues/59): Maximum values in statistics can be incorrect
  * fix [issue 62](../../issues/62): IndexOutOfBounds for report on some jobs
...
### 1.21.0 ###
  ...
  * fix [issue 48](../../issues/48): Quartz errors for persistent jobs after jobs' classes are renamed or deleted
...
[JavaMelody Grails Plugin](http://www.grails.org/plugin/grails-melody) 0.5 has been released.
  ...
  * [issue 36](../../issues/36) is a known open issue for the plugin when a closure is used in a service (contribution welcome)
...
### 1.19.0 ###
  ...
  * fix [issue 38](../../issues/38): Distributed (XA) Transactions aborted by 'rollback' in JavaInformations.java
  * fix [issue 41](../../issues/41): NumberFormatException requesting process information
  * fix [issue 44](../../issues/44): Sql requests not monitored when using jdk1.6, a recent jdbc driver and net.bull.javamelody.JdbcDriver
...
### 1.18.0 ###
  ...
  * fix [issue 33](../../issues/33): Database name or jdbc driver with '#' causes IndexOutOfBounds
  * fix [issue 34](../../issues/34): Application server restart causes persistent Quartz jobs to be hanging around
  * added: "JNDI Tree" link in "system actions" (for example to check if a datasource is really in "java:/" or in "java:/comp/env/")
  * install note for bamboo 2.6: see end of [issue 37](../../issues/37)
...
### 1.17.0 ###
  ...
  * fix [issue 28](../../issues/28): NoSuchMethodError when undeployed and when JavaMelody 1.15.0 or 1.16.0 is used with Quartz 1.7 or 1.8
  * fix [issue 30](../../issues/30): Quartz job CRON expression with '#' causes NullPointerException
  * fix for Grails: an exception in initialisation of Quartz should not stop initialisation of the webapp
  * fix [issue 8](../../issues/8): A synchronized (rrdDb) was added against the exception "Bad sample timestamp x. Last update time was x, at least one second step is required". It was seen with deployment on Tomcat without modification of monitored webapps.
  * fix for JIRA: "mean sql hits" and "mean sql time" displayed in http and in jsp statistics
  * changed: The default order of MonitoringSpringAdvisor is now 0 for priority before other spring advisors ([issue 32](../../issues/32))
...
### 1.16.0 ###
  ...
  * Fix [issue 26](../../issues/26): In JIRA, the count of used jdbc connections increases in the display with each "/monitoring" page refresh (but connections are really closed)
...
[JavaMelody Grails Plugin](http://www.grails.org/plugin/grails-melody) 0.3 has been released by _Liu Chao_.
  ...
  * fix [issue 18](../../issues/18): Grails-melody plugin breaks the filterpane plugin
...
### 1.15.1 of Hudson plugin and of JIRA/Bamboo plugin ###
  ...
  * **[JIRA/Bamboo plugin](https://plugins.atlassian.com/plugin/details/20909)**
    * fix [issue 24](../../issues/24): StackOverflow when a SSO plugin with seraph is used
...
### 1.15.0 ###
  ...
  * fix [issue 20](../../issues/20): SQL requests were not monitored and "jdbc connections" were always 0, when **JndiObjectFactoryBean** is used in Spring
  * fix [issue 21](../../issues/21) for **JIRA**: Null pointer with quartz jobs in jira
  * fix [issue 22](../../issues/22) for **JIRA**: The report will be visible only for jira administrators
  * fix [issue 23](../../issues/23) for **JIRA**: The report displayed only random characters when gzip compression is enabled in jira
  * [doc updated](UserGuide#10._Batch_jobs_(if_Quartz).md) for **quartz** jobs scheduled with Spring, with help from _Pether Sörling_.
  * [doc updated](UserGuide#3._First_results.md) for [issue 19](../../issues/19) InternalError "Can't connect to window server" on Mac OS X Server.
...
### 1.14.0 ###
  ...
  * fix [issue 16](../../issues/16): InternalError "Unable to open directory /proc/self/fd" on ubuntu or debian using the tomcat package with jsvc
...
### 1.13.0 ###
  ...
  * fix [issue 14](../../issues/14): html reports in utf-8 for cyrillic and other characters
  * fix [issue 15](../../issues/15): support for log4j-over-slf4j
...
### 1.12.0 ###
  ...
  * if a centralized collect server is used:
    * fix [issue 12](../../issues/12): workaround for a bug in Winstone (standalone mode)
    * fix [issue 13](../../issues/13): NoClassDefFoundError with ehcache in the collect server if the monitored application uses ehcache
...
### 1.9.0 ###
  ...
  * fix [issue 3](../../issues/3) "http status 500" : NoSuchElementException in getPID on Solaris 10
